				...
				if ((c++, 1 === t.nodeType)) {
					if (t.hasAttributes()) {
						...
						for (; i-- > 0; ) {
							...
							t.removeAttribute(i);
							const o = r.split(n);
							this.parts.push({ type: 'attribute', index: c, name: s, strings: o }), (u += o.length - 1);
						}
					}
					...
					if (e.indexOf(s) >= 0) {
						...
						for (let e = 0; e < a; e++) {
							...
							s.insertBefore(i, t), this.parts.push({ type: 'node', index: ++c });
						}
						'' === r[a] ? (s.insertBefore(h(), t), i.push(t)) : (t.data = r[a]), (u += a);
					}
				} else if (8 === t.nodeType)
					if (t.data === s) {
						const e = t.parentNode;
						(null !== t.previousSibling && c !== d) || (c++, e.insertBefore(h(), t)),
							(d = c),
						...
						let e = -1;
						for (; -1 !== (e = t.data.indexOf(s, e + 1)); ) this.parts.push({ type: 'node', index: -1 }), u++;
					}
...
function d(t, e) {
	...
	for (; n.nextNode(); ) {
		...
		for (
			t.previousSibling === d && (d = null), e.has(t) && (l.push(t), null === d && (d = t)), null !== d && h++;
			void 0 !== o && o.index === a;
			...
			if (((r = i[o]), a(r))) {
				for (; h < r.index; )
					h++,
						...
						null === (l = n.nextNode()) && ((n.currentNode = s.pop()), (l = n.nextNode()));
				if ('node' === r.type) {
...
class v {
	...
	getHTML() {
		...
		for (let r = 0; r < t; r++) {
			...
			const a = l.exec(t);
			e += null === a ? t + (n ? f : i) : t.substr(0, a.index) + a[1] + a[2] + '$lit$' + a[3] + s;
		}
...
class P {
	...
	setValue(t) {
		t === g || (S(t) && t === this.value) || ((this.value = t), _(t) || (this.committer.dirty = !0));
	}
	commit() {
		for (; _(this.value); ) {
			const t = this.value;
...
class N {
	...
	insertAfterPart(t) {
		t.__insert((this.startNode = h())), (this.endNode = t.endNode), (t.endNode = this.startNode);
	}
	setValue(t) {
	...
	__commitIterable(t) {
		...
		for (const n of t)
			(s = e[i]),
				void 0 === s &&
					((s = new N(this.options)), e.push(s), 0 === i ? s.appendIntoPart(this) : s.insertAfterPart(e[i - 1])),
				s.setValue(n),
				s.commit(),
				i++;
...
class C {
	constructor(t, e, s) {
		if (((this.value = void 0), (this.__pendingValue = void 0), 2 !== s.length || '' !== s[0] || '' !== s[1]))
 ...
 */ function M(t) {
	let e = V.get(t.type);
	void 0 === e && ((e = { stringsArray: new WeakMap(), keyString: new Map() }), V.set(t.type, e));
...
const I = (t) => (e) => {
		const i = q(e.type, t);
		let n = V.get(i);
		void 0 === n && ((n = { stringsArray: new WeakMap(), keyString: new Map() }), V.set(i, n));
		...
		if (((o = n.keyString.get(a)), void 0 === o)) {
			...
			H && window.ShadyCSS.prepareTemplateDom(s, t), (o = new r(e, s)), n.keyString.set(a, o);
		}
	...
	j = (t, e, s) => {
		...
		s
			? (function (t, e, s = null) {
					...
					for (; r.nextNode(); ) {
						h++;
						for (r.currentNode === s && ((a = c(e)), s.parentNode.insertBefore(e, s)); -1 !== o && n[o].index === h; ) {
							if (a > 0) {
...
class J extends HTMLElement {
	...
	static createProperty(t, e = D) {
		if (
			(this._ensureClassProperties(), this._classProperties.set(t, e), e.noAccessor || this.prototype.hasOwnProperty(t))
		)
			return;
	...
	performUpdate() {
		...
		t &&
			(1 & this._updateState || ((this._updateState = 1 | this._updateState), this.firstUpdated(e)), this.updated(e));
	}
	_markUpdated() {
...
class Y {
	constructor(t, e) {
		if (e !== X) throw new Error('CSSResult is not constructable. Use `unsafeCSS` or `css` instead.');
		this.cssText = t;
	}
	get styleSheet() {
		return (
...
class Q extends J {
	...
	static _getUniqueStyles() {
		...
		if (void 0 === t) this._styles = [];
			...
			const e = (t, s) => t.reduceRight((t, s) => (Array.isArray(s) ? e(s, t) : (t.add(s), t)), s),
				s = e(t, new Set()),
				i = [];
	...
	update(t) {
		const e = this.render();
		super.update(t),
			e !== K && this.constructor.render(e, this.renderRoot, { scopeName: this.localName, eventContext: this }),
	...
	(Q.render = (t, s, i) => {
		...
		if (
			(((t, s, i) => {
				let n = R.get(s);
				void 0 === n &&
					(e(s, s.firstChild), R.set(s, (n = new N(Object.assign({ templateFactory: M }, i)))), n.appendInto(s)),
					n.setValue(t),
					n.commit();
...
customElements.define(
	...
	class extends Q {
		...
		get _showHandle() {
			return !this.readonly && (null != this.value || (null != this.high && null != this.low));
		}
		_angleInside(t) {
			let e = ((this.startAngle + this.arcLength / 2 - t + 180 + 360) % 360) - 180;
			return e < this.arcLength / 2 && e > -this.arcLength / 2;
		}
		_angle2xy(t) {
			return this.rtl ? { x: -Math.cos(t), y: Math.sin(t) } : { x: Math.cos(t), y: Math.sin(t) };
		}
		_xy2angle(t, e) {
			return this.rtl && (t = -t), (Math.atan2(e, t) - this._start + 2 * Math.PI) % (2 * Math.PI);
		}
		_value2angle(t) {
			const e = ((t = Math.min(this.max, Math.max(this.min, t))) - this.min) / (this.max - this.min);
			return this._start + e * this._len;
		}
		_angle2value(t) {
			return Math.round(((t / this._len) * (this.max - this.min) + this.min) / this.step) * this.step;
		}
		get _boundaries() {
		...
		dragStart(t) {
			...
			if ((e.classList.contains('overflow') && (e = e.nextElementSibling), !e.classList.contains('handle'))) return;
			e.setAttribute('stroke-width', 2 * this.handleSize * this.handleZoom * this._scale);
			const i = 'high' === e.id ? this.low : this.min,
				n = 'low' === e.id ? this.high : this.max;
			(this._rotation = { handle: e, min: i, max: n, start: this[e.id], type: t.type, cooldown: s }),
				(this.dragging = !0);
		}
		_cleanupRotation() {
		...
		dragEnd(t) {
			...
			let s = new CustomEvent('value-changed', { detail: { [e.id]: this[e.id] }, bubbles: !0, composed: !0 });
			this.dispatchEvent(s),
				this.low && this.low >= 0.99 * this.max ? (this._reverseOrder = !0) : (this._reverseOrder = !1);
		}
		drag(t) {
			if (!this._showHandle || this.disabled) return;
			if (!this._rotation) return;
			if (this._rotation.cooldown) return window.clearTimeout(this._rotation.coldown), void this._cleanupRotation();
		...
		_dragpos(t) {
			...
			const e = this._rotation.handle;
			this[e.id] = t;
			let s = new CustomEvent('value-changing', { detail: { [e.id]: t }, bubbles: !0, composed: !0 });
			this.dispatchEvent(s);
		}
		_keyStep(t) {
			if (!this._showHandle || this.disabled) return;
			...
			('ArrowLeft' !== t.key && 'ArrowDown' !== t.key) ||
				(t.preventDefault(), this.rtl ? this._dragpos(this[e.id] + this.step) : this._dragpos(this[e.id] - this.step)),
				('ArrowRight' !== t.key && 'ArrowUp' !== t.key) ||
					(t.preventDefault(),
					this.rtl ? this._dragpos(this[e.id] - this.step) : this._dragpos(this[e.id] + this.step)),
				'Home' === t.key && (t.preventDefault(), this._dragpos(this.min)),
				'End' === t.key && (t.preventDefault(), this._dragpos(this.max));
		}
		firstUpdated() {
			document.addEventListener('mouseup', this.dragEnd.bind(this)),
				document.addEventListener('touchend', this.dragEnd.bind(this), { passive: !1 }),
				document.addEventListener('mousemove', this.drag.bind(this)),
				document.addEventListener('touchmove', this.drag.bind(this), { passive: !1 }),
				document.addEventListener('keydown', this._keyStep.bind(this));
		}
		updated(t) {
			if (this.shadowRoot.querySelector('.slider')) {
				const t = window.getComputedStyle(this.shadowRoot.querySelector('.slider'));
				if (t && t.strokeWidth) {
					...
					if (e > this.handleSize * this.handleZoom) {
						const t = this._boundaries,
							s = `\n          ${(e / 2) * Math.abs(t.up)}px\n          ${(e / 2) * Math.abs(t.right)}px\n          ${
								(e / 2) * Math.abs(t.down)
							}px\n          ${(e / 2) * Math.abs(t.left)}px`;
						this.shadowRoot.querySelector('svg').style.margin = s;
					}
				}
			}
			if (this.shadowRoot.querySelector('svg') && void 0 === this.shadowRoot.querySelector('svg').style.vectorEffect) {
				t.has('_scale') &&
					1 != this._scale &&
					this.shadowRoot
						.querySelector('svg')
						...
						.forEach((t) => {
							...
							const e = parseFloat(getComputedStyle(t).getPropertyValue('stroke-width'));
		...
		_renderHandle(t) {
			const e = this._value2angle(this[t]),
				s = this._angle2xy(e),
				i = { value: this.valueLabel, low: this.lowLabel, high: this.highLabel }[t] || '';
			return O`
      <g class="${t} handle">
        <path
          id=${t}
      ...
      >
        <g class="slider">
          ...
          <path
            ...
            vector-effect="non-scaling-stroke"
            d=${this._renderArc(
							this._value2angle(null != this.low ? this.low : this.min),
							this._value2angle(null != this.high ? this.high : this.value)
						)}
          />
          <path
            class="shadowpath"
        ...
        ${
					this._showHandle
						? null != this.low
							? this._reverseOrder
								? U`${this._renderHandle('high')} ${this._renderHandle('low')}`
								: U`${this._renderHandle('low')} ${this._renderHandle('high')}`
							: U`${this._renderHandle('value')}`
						: ''
				}
        </g>
      </svg>
