...
function smoothToPricing(e) {
	document.getElementById(e) && document.getElementById(e).scrollIntoView({ behavior: 'smooth' });
}
function debounce(l, a, r) {
	var o;
	return function () {
...
var myLatlng,
	...
	map,
	marker,
	popoverList = (popoverTriggerList = [].slice.call(document.querySelectorAll('[data-toggle="popover"]'))).map(
		function (e) {
			return new bootstrap.Popover(e);
		}
	),
	tooltipList = (tooltipTriggerList = [].slice.call(document.querySelectorAll('[data-toggle="tooltip"]'))).map(
		function (e) {
			return new bootstrap.Tooltip(e);
		}
	);
	...
	if (!document.querySelector('.dropdown-hover')) {
		...
		for (var t = e.parentElement.parentElement.children, l = 0; l < t.length; l++)
			t[l].lastElementChild != e.parentElement.lastElementChild &&
				(t[l].lastElementChild.classList.remove('show'), t[l].firstElementChild.classList.remove('show'));
		e.nextElementSibling.classList.contains('show')
			? (e.nextElementSibling.classList.remove('show'), e.classList.remove('show'))
	...
	if ((shadowCardsRounded = document.querySelectorAll('.blur-shadow-avatar.rounded-circle')))
		for (i = 0; i < shadowCardsRounded.length; i++) {
			for (
				...
				shadowCardsRounded[i].firstElementChild.classList.contains(avatarClasses[k]) &&
...
document.querySelector('#google-maps') &&
	((mapOptions = {
		...
		styles: [
			...
			{ featureType: 'water', elementType: 'all', stylers: [{ color: '#C5CBF5' }, { visibility: 'on' }] },
		],
	}),
	(map = new google.maps.Map(document.getElementById('google-maps'), mapOptions)),
	(marker = new google.maps.Marker({ position: myLatlng, title: 'Hello World!' })).setMap(map));
...
function copyCode(e) {
	...
	l.selectNodeContents(r), t.removeAllRanges(), t.addRange(l), document.execCommand('copy');
			...
			if (r) {
				...
				o.querySelector('li:nth-child(' + a + ') .nav-link').onclick = function () {
					...
					if (o.classList.contains('flex-column')) {
						for (var t = 1; t <= l.indexOf(r); t++) e += o.querySelector('li:nth-child(' + t + ')').offsetHeight;
						(s.style.transform = 'translate3d(0px,' + e + 'px, 0px)'),
							(s.style.height = o.querySelector('li:nth-child(' + t + ')').offsetHeight);
					} else {
						for (t = 1; t <= l.indexOf(r); t++) e += o.querySelector('li:nth-child(' + t + ')').offsetWidth;
		...
		total.forEach(function (l, e) {
			...
			if (o) {
				...
				if (l.classList.contains('flex-column')) {
					for (var s = 1; s <= e.indexOf(o); s++) t += l.querySelector('li:nth-child(' + s + ')').offsetHeight;
					(a.style.transform = 'translate3d(0px,' + t + 'px, 0px)'),
						(a.style.width = l.querySelector('li:nth-child(' + r + ')').offsetWidth + 'px'),
						(a.style.height = l.querySelector('li:nth-child(' + s + ')').offsetHeight);
				} else {
					for (s = 1; s <= e.indexOf(o); s++) t += l.querySelector('li:nth-child(' + s + ')').offsetWidth;
